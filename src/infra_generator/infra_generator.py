...
class InfraGenerator:
    ...
    def _detect_services_and_versions(self, manifest_files: list) -> list:
        ...
        t0 = time.time()
        logger.info(f"Received {len(manifest_files)} manifest files for service detection.")
        services = []
        for mf in manifest_files:
            ...
            logger.info(f"LLM manifest parse for {mf_path} took {time.time()-t_llm:.1f}s")
            try:
                parsed = json.loads(result)
            except Exception:
                parsed = {"language": "unknown", "version": None, "service_name": os.path.basename(os.path.dirname(mf_path)) or "root_service"}
            services.append({
                "name": parsed.get("service_name", os.path.basename(os.path.dirname(mf_path)) or "root_service"),
        ...
        logger.info(f"Service/manifest detection for {len(manifest_files)} manifest(s) took {time.time()-t0:.1f}s")
        return services
    ...
    def _gather_context_for_prompt(self, retriever: Retriever, summary: str = None, tree: list = None, max_chars: int = 8000) -> dict:
        ...
        entry_candidates = [f for f in (tree or []) if any(x in f for x in ["app.py", "main.py", "server.js", "index.js", "main.go"])]
        if entry_candidates:
        ...
        logger.info(f"Retriever query for additional code snippets took {time.time()-t_retrieve:.1f}s")
        snippets = []
        current_len = 0
    ...
    def _invoke_llm(self, system_prompt_template: str, user_prompt_template: str, context: dict) -> str:
        ...
        if content.startswith("```") and content.endswith("```"):
            ...
            content = re.sub(r'\n```$', '', content)
        logger.info(f"LLM invocation for prompt '{system_prompt_template[:30]}...' took {time.time()-t0:.1f}s")
        return content
    ...
    def _clean_docker_output(self, content: str) -> str:
        """
        Clean LLM output to remove markdown/code block wrappers, #GOOD/#BAD comments, and any non-Dockerfile/Compose content.
        Handles triple backticks and language specifiers robustly.
        """
    ...
    def generate_generic_infrastructure(self, prompt: str, retriever: Retriever, tree: list = None, summary: str = None) -> str:
        context = self._gather_context_for_prompt(retriever, summary=summary, tree=tree)
