...
def llm_parse_tree(tree_str: str, config: dict) -> list:
    """
    Use the LLM to parse a pretty-printed directory tree string into a list of relative manifest file paths only.
    """
    from langchain_ollama import ChatOllama
    manifest_filenames = [
        "package.json", "requirements.txt", "pyproject.toml", "Pipfile", "environment.yml",
        "setup.py", "poetry.lock", "yarn.lock", "Dockerfile", "go.mod", "go.sum", "Gemfile",
        "composer.json", "Cargo.toml", "Cargo.lock", "build.gradle", "pom.xml"
    ]
    ...
    prompt = f"""
    Given the following directory tree (as output by the `tree` command), extract and return a JSON list of all file paths (relative to the root) that are manifest files. Manifest files are any files with the following names: {manifest_patterns}.
    Do not include any other files or directories. Output only the JSON list, nothing else.
...
def run_infra_pipeline(
    ...
    logger.info(f"Project summary ingestion complete. Time taken: {time.time()-t_ingest:.1f}s")
    ...
    if isinstance(tree, str):
        ...
        logger.info(f"LLM parsed {len(all_file_paths)} manifest file paths from pretty tree.")
    ...
    logger.info(f"Service detection complete. Found {len(services)} service(s). Time taken: {time.time()-t_detect:.1f}s")
    if not services:
        raise RuntimeError(f"Could not detect any services in {source}. Check for manifest files (e.g., package.json, requirements.txt).")
    ...
    for svc in services:
        ...
        svc_ctx = query_handler.build_context(
            query=f"code snippets for {svc['name']} {svc['language']} service entrypoint, server setup, and dependencies",
        ...
        logger.info(f"Dockerfile for '{svc['name']}' generated in {time.time()-t_docker:.1f}s")
    ...
    repo_ctx = query_handler.build_context(
        query="top-level configuration, docker-compose examples, inter-service communication",
        k=config.get("rag_k", 5),
        project=project_name,
        max_context_length=config.get("max_code_context_chars", 2000),
    )
    ...
    logger.info(f"Infrastructure generation pipeline finished successfully. Total time: {time.time()-t0:.1f}s")
